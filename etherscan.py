...
class Client():
    ...
    def __internalTransaction(self, source: dict):
        """Repack the __transaction dict"""
        return {
            'timestamp': self.__int(source['timeStamp']),
            'block_number': self.__int(source['blockNumber']),
            ...
            'is_error': self.__bool(source['isError']),
            'error_code': self.__str(source['errCode']),
            'trace_id': self.__bool(source['traceId']),

            'contract_address': self.__str(source['contractAddress']),
            'type': self.__str(source['type']),
        }
    ...
    def get_transactions_by_address(self,
        ...
        for t in rs:
            if(tx_type == 'normal'):
                transactions.append(self.__normalTransaction(t))
            elif(tx_type == 'internal'):
                transactions.append(self.__internalTransaction(t))
            else:
                raise Exception('param `type` must be "normal" or "internal"')
